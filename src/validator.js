const validator = {
  // defino la función validate que debe retornar true o false
  ...
  isValid: function (creditCardNumber) {
    ...
    */
    ...
    let result = 0;
    for (let i = 0; i < reverseArrayCreditCardNumber.length; i++) {
      //recorro el array para realizar la siguiente verificación de cada valor del array
      if (i % 2 !== 0) {
        const impar = reverseArrayCreditCardNumber[i] * 2; // a los valores de los indices impares (posición par) los multiplico por 2
        if (impar >= 10) {
          //estableco una segunda condición: si el resultado es mayor o igual a 10
    ...
    if (result % 10 === 0) {
      //operación para verificar que el resultado es múltiplo de 10
  ...
  maskify: function (creditCardNumber) {
    const mascara =
      ...
      creditCardNumber.slice(-4); // enmascaramiento
    return mascara;
  },
};
